...
const CalendarRoundCard: FC<{
  ...
  return (
    <section
      className={`flex min-h-0 min-w-0 flex-col overflow-hidden rounded-sm border border-[var(--foreground)]/10 bg-[var(--row-a)] ${className ?? ""}`}
    >
      <header className="flex shrink-0 flex-col items-center justify-center gap-0.5 bg-[var(--panel)] px-1.5 py-1 text-center">
        <div className="text-[0.6875rem] font-bold leading-none sm:text-xs">
          Тур {round}
        </div>
        {date && (
          <small className="muted text-[0.5625rem] leading-tight sm:text-[0.625rem]">
            {date}
          </small>
        )}
      </header>
      <ul className="flex min-h-0 flex-1 flex-col overflow-hidden">
        {games.map((game, index) => (
          <li
            ...
            className={`grid grid-cols-[2fr_1fr_2fr] items-center gap-x-1 px-1.5 py-0.5 text-[0.5625rem] leading-tight sm:gap-x-1.5 sm:px-2 sm:py-1 sm:text-[0.625rem] ${
